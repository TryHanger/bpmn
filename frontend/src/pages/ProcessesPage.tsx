...
function buildDefaultVariables(currentUserId: string): ProcessVariableRow[] {
  return [
    { name: 'amount', value: '75', type: 'integer' },
    { name: 'initiatorUserId', value: currentUserId, type: 'string', locked: true },
  ]
}
...
export function ProcessesPage() {
  ...
  const handleVariableChange = (index: number, value: string) => {
    setVariables((current) => current.map((row, rowIndex) => (rowIndex === index ? { ...row, value } : row)))
  }
          ...
          >
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-slate-400">value как integer, initiatorUserId подставляется автоматически</span>
              </div>

              <div className="space-y-3">
                {variables.map((row, index) => (
                  <div key={row.name} className="grid grid-cols-12 gap-2">
                    <input value={row.name} readOnly className="col-span-4 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-500" />
                    {row.type === 'integer' ? (
                      <input
                        type="number"
                        value={row.value}
                        onChange={(event) => handleVariableChange(index, event.target.value)}
                        className="col-span-5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm"
                        required
                      />
                    ) : (
                      <input
                        ...
                        readOnly={row.locked}
                        className={[
                          'col-span-5 rounded-xl border px-3 py-2 text-sm',
                          row.locked ? 'border-slate-200 bg-slate-100 text-slate-500' : 'border-slate-200 bg-slate-50',
                        ].join(' ')}
                      />
                    )}
                    <input value={row.type} readOnly className="col-span-3 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-500" />
                  </div>
                ))}
              </div>
            </div>
